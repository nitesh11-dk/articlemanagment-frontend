...
const MyArticles = () => {
  ...
  const fetchArticles = useCallback(async () => {
    ...
    try {
      setLoading(true);
      const res = await axios.get(`${API_URL}/articles/user/${user.id}`);
  ...
  if (!user || (user.role !== 'editor' && user.role !== 'admin')) {
    return (
      <div className="flex flex-col items-center justify-center py-24 text-center">
        <Info className="w-16 h-16 text-danger mb-4" />
        <h2 className="text-2xl font-bold text-text-primary">Access Denied</h2>
        <p className="text-text-secondary max-w-sm">Only Editors and Admins can access this page.</p>
      </div>
    );
  }
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-7xl">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-10">
        <div>
          <h1 className="text-3xl font-extrabold text-text-primary tracking-tight mb-2">My Articles</h1>
        ...
        <button 
          ...
          className="w-full md:w-auto h-12 bg-primary text-white font-bold px-8 rounded-xl shadow-lg shadow-primary/20 hover:bg-primary-hover hover:-translate-y-0.5 transition-all flex items-center justify-center gap-2"
        >
      ...
      {loading ? (
        ...
        <div className="text-center py-24 bg-zinc-50 rounded-3xl border-2 border-dashed border-zinc-200">
          ...
          <h2 className="text-xl font-bold text-zinc-400 mb-1">No articles yet</h2>
          <p className="text-sm text-zinc-400">Create your first article to get started.</p>
        </div>
      )}
